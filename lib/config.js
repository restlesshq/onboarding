...
// Mirrors MAX_OAS_BYTES in the dashboard's api/projects/[projectId]/oas route,
// so `register` can refuse an unuploadable spec instead of `login` doing it
// after the user has already been told setup worked.
export const MAX_OAS_BYTES = 8 * 1024 * 1024;
...
// Where the telemetry notice and `restless telemetry status` point people.
// See `lib/telemetry.js` for what is actually collected, and `docs/telemetry.md`
// for the copy that ships in the tarball.
export const TELEMETRY_DOCS_URL = 'https://restless.ai/docs/telemetry';
