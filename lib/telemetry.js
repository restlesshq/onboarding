 ...
 *
 * Modelled on Vercel CLI Telemetry - opt-out, enum-only payloads, and a
 * debug mode that prints what would be sent instead of sending it. Unlike
 * Vercel's, it prints no first-run notice: disclosure lives in the README,
 * `docs/telemetry.md`, and `restless telemetry status`, and nothing about
 * telemetry is written to the terminal on an ordinary run.
 *
...
import { loadConfig, updateConfig, anonymousId, storedAnonymousId } from './user-config.js';
...
export const OAS_SOURCE_KINDS = new Set([
  'ai', 'native', 'found', 'file', 'url', 'describe', 'combined', 'agent',
]);
...
function resolveMode() {
  ...
  // Below this line is the one "not you" gate, overridable, because it is not
  // a statement about what the user wants: our own checkouts and `npm link`ed
  // copies would otherwise pollute the numbers with our own runs.
...
function normalizeFramework(v) {
  ...
  const flat = v.trim().toLowerCase().replace(/[\s_.-]+/g, '');
  // "Next.js" and "Express.js" name `next` and `express`; `nestjs` keeps its suffix.
  const bare = flat.endsWith('js') ? flat.slice(0, -2) : flat;
  return !FRAMEWORKS.has(flat) && FRAMEWORKS.has(bare) ? bare : flat;
}
...
export function buildPayload({ exitCode = null, outcome = 'ok', summary = null } = {}) {
  return {
    ...
    // Debug mode only prints, so it must not be what writes an id to disk.
    anonymousId: state.mode === 'debug' ? (storedAnonymousId() || crypto.randomUUID()) : anonymousId(),
