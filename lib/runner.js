...
export function createPlanManager() {
  ...
  function markActiveFailed() {
    ...
    if (statuses[activeStep] !== 'failed') {
      const startedAt = stepStartedAt[activeStep] || 0;
      const failedMs = startedAt ? Date.now() - startedAt : null;
      debug.log('step.failed', { index: activeStep, label: STEPS[activeStep].label, durationMs: failedMs });
      telemetry.recordStep(activeStep, 'failed', failedMs);
    }
